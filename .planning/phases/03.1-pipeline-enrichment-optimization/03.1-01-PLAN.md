...
3. Create `scripts/pipeline/lib/ingredient-extractor.ts`:
   ...
   - Export `async function extractUniqueIngredients(scrapedPath: string): Promise<string[]>`
     ...
     - Collect all ingredient names into a Set (exact string match, no normalization -- Jow names are consistent with 0 case-only duplicates)
...
```
   ...
   - IMPORTANT: Validate that the returned ingredient count matches the input count. If not, throw an error: `"Expected ${ingredientNames.length} ingredients, got ${result.length}"`
...
5. Export: `enrichIngredientBatch`, `enrichIngredientBatchWithRetry` (add to existing exports). Keep existing exports unchanged.
  ...
  <verify>
Run `pnpm check` to confirm no lint/format errors. Verify the new exports are accessible:
```bash
