...
2. `assembleEnrichedRecipe(recipe: ScrapedRecipe, macroLookup: Map<string, EnrichedIngredient>): { enriched: EnrichedRecipe; missingIngredients: string[] } | null`
   ...
   - If SOME ingredients are missing, still assemble the recipe but return the list of missing ingredient names for logging. For missing ingredients, skip them from enrichedIngredients (do NOT include null macros -- the upload API expects valid EnrichedIngredient objects)
   - Build `EnrichedRecipe` object: `{ ...recipe, enrichedIngredients }` where `enrichedIngredients` is the array of looked-up `EnrichedIngredient` objects
   - Return the assembled recipe and list of missing ingredient names
...
5. For each recipe:
   ...
   - Call `crossValidateNutrition(enrichedRecipe.enrichedIngredients, recipe)` for quality check
