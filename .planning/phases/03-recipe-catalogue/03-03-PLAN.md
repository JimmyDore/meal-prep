...
<task type="auto">
  ...
  <action>
    1. Create src/app/recipes/[id]/page.tsx (Server Component):
       ...
       **Macros per serving section:**
       ...
       - Display jowNutritionPerServing data. This is JSONB -- the structure from the Jow scraper stores nutritionalFacts as an array of {id, amount}. The enrichment pipeline may have structured it differently.
       - Parse the JSONB and display: Calories (kcal), Proteines (g), Glucides (g), Lipides (g) using MacroBadge components
       - If jowNutritionPerServing is null or empty, show "Macros non disponibles"
       - Wrap in a flex row with gap
  ...
  <done>
    Recipe detail page renders all recipe data: image, title, tags, time/difficulty/servings, macros per serving, ingredients with quantities and per-100g macros, description, and Jow external link. Not-found page handles missing recipes. Loading skeleton shows during data fetch.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    5. Verify: macros per serving section shows calories, protein, carbs, fat values
...
5. Back link returns to catalogue
6. Jow link opens in new tab
...
- Macros per serving displayed (calories, protein, carbs, fat) using MacroBadge
